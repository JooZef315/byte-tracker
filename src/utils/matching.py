...
from .ops import bbox_ioa, batch_probiou
...
try:
    ...
    raise ImportError(
        "lap package is required for tracking. Install it with: pip install lap>=0.5.12"
    )
...
def linear_assignment(cost_matrix: np.ndarray, thresh: float, use_lap: bool = True):
    ...
    Returns:
        matched_indices (list[list[int]] | np.ndarray): Array of matched indices of shape (K, 2), where K is the number of matches.
